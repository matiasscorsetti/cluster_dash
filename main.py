...
def displayResultst(resultsbutton):
    ...
    if 'results-button' in changed_id:
        if all(v is not None for v in [df, metrics]):
            ...
            silhoutte_class_data = metrics.get("cluster_silhouette")
            bar_silhoutte_class = dcc.Graph(figure=graphics.bar_from_dict(dictionary=silhoutte_class_data))
            pie_silhouette_clas = dcc.Graph(figure=graphics.pie_from_dict(dictionary=silhoutte_class_data))

        else:
            msg = "a dataset has not been loaded"

    return msg, distribution_title, bar_distribution, pie_distribution, silhouette_title, bar_silhoutte_class, pie_silhouette_clas
...
# results metrics
@app.callback(
    [Output('container-results-metric', 'children'),
     Output('bar-silhouette-coefficient-class2', 'children'),
     ],
    [Input('results-metrics-button', 'n_clicks'),
     ],
    )
def displayResultsMetrics(resultsmetricsbutton):
    ...
    return msg, bar_silhoutte_class
